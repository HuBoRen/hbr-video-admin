...
var recoveryVideo = function(videoId) {
	
	var flag = window.confirm("是否恢复播放");
	if (!flag) {
		return;
	}
	
	$.ajax({
    	url: $("#hdnContextPath").val() + "/video/recoveryVideo.action?videoId=" + videoId,
    	type: "POST",
    	async: false,
    	success: function(data) {
            if(data.status == 200 && data.msg == "OK") {
...
var UsersReportsList = function () {
	...
	var handleUsersReportsList = function() {
        ...
        jqGrid.jqGrid({  
            ...
            datatype: "json",  
            colNames: ['ID', '举报类型', '举报内容', '被举报人', '被举报视频id', '被举报视频', '视频状态', '提交用户', '举报日期', "操作1","操作2"],  
            colModel: [  
                ...
                { name: '', index: '', width: 25, sortable: false, hidden: false,
                	formatter:function(cellvalue, options, rowObject) {
			    ...
			    { name: '', index: '', width: 25, sortable: false, hidden: false,
                	formatter:function(cellvalue, options, rowObject) {
                		var button1 = '<button class="btn btn-outline blue-chambray" id="" onclick=recoveryVideo("' + rowObject.dealVideoId + '") style="padding: 1px 3px 1px 3px;">恢复播放</button>';
			    		return button1;
