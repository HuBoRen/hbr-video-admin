var delUser = function(userId) {
	...
	var flag = window.confirm("是否永久封禁");
	if (!flag) {
		return;
	}
	
	$.ajax({
    	url: $("#hdnContextPath").val() + "/users/delUser.action?userId=" + userId,
    	type: "POST",
    	async: false,
...
var List = function () {
	...
	var handleList = function() {
        ...
        jqGrid.jqGrid({  
            ...
            datatype: "json",  
            colNames: ['ID', '头像', '用户名', '昵称', '粉丝数', '关注数', '获赞数','操作'],  
            colModel: [  
                ...
                { name: '', index: '', width: 20, sortable: false, hidden: false,
                	formatter:function(cellvalue, options, rowObject) {
                		var button = '<button class="btn btn-outline blue-chambray" id="" onclick=delUser("' + rowObject.id + '") style="padding: 1px 3px 1px 3px;">封禁用户</button>';
			    		return button;
			    	}
			    }
            ],  
